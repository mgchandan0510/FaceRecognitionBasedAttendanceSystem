...
#### Calculate accuracy of the face recognition model
def calculate_accuracy():
    test_faces = []
    test_labels = []
    test_userlist = os.listdir('static/test_faces')
    for user in test_userlist:
        for imgname in os.listdir(f'static/test_faces/{user}'):
            img = cv2.imread(f'static/test_faces/{user}/{imgname}')
            resized_face = cv2.resize(img, (50, 50))
            test_faces.append(resized_face.ravel())
            test_labels.append(user)
    test_faces = np.array(test_faces)

    predicted_labels = identify_face(test_faces)
